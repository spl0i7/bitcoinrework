...
static const struct {
    ...
    int flags;
        ...
        {"ALL|BTCRW_FORK", SIGHASH_ALL | SIGHASH_BTCRW_FORK},
        {"NONE|BTCRW_FORK", SIGHASH_NONE | SIGHASH_BTCRW_FORK},
        {"SINGLE|BTCRW_FORK", SIGHASH_SINGLE | SIGHASH_BTCRW_FORK},
        {"ALL|BTCRW_FORK|ANYONECANPAY", SIGHASH_ALL | SIGHASH_BTCRW_FORK | SIGHASH_ANYONECANPAY},
        {"NONE|BTCRW_FORK|ANYONECANPAY", SIGHASH_NONE | SIGHASH_BTCRW_FORK | SIGHASH_ANYONECANPAY},
        {"SINGLE|BTCRW_FORK|ANYONECANPAY", SIGHASH_SINGLE | SIGHASH_BTCRW_FORK | SIGHASH_ANYONECANPAY},
};
...
{
    int nHashType = SIGHASH_ALL | SIGHASH_BTCRW_FORK;
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                newcoin.out.SetValue(0);
                if (prevOut.exists("amount")) {
                    newcoin.out.SetValue(AmountFromValue(prevOut["amount"]));
                }
                newcoin.nHeight = 1;
    ...
    bool fHashSingle = ((nHashType & ~(SIGHASH_ANYONECANPAY | SIGHASH_BTCRW_FORK)) == SIGHASH_SINGLE);
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        const CAmount& amount = coin.GetValue(Params().GetConsensus().BTCRWForkHeight);
