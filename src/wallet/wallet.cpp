...
CAmount CWalletTx::GetCredit(const isminefilter& filter) const
{
    ...
    if (filter & ISMINE_SPENDABLE)
    {
        ...
        else
        {
            ...
            if(!Params().IsBTCRWForkEnabled(GetTxConfirmHeight())){
    ...
    unsigned int nBytes;
    {
        ...
        if (sign)
        {
            ...
            for (const auto& coin : setCoins)
            {
                ...
                SignatureData sigdata;
                int nHashTypeIn = 0;
                if(Params().IsBTCRWForkEnabled(chainActive.Height())) {
                    nHashTypeIn = SIGHASH_ALL | SIGHASH_BTCRW_FORK;
                }else {
...
CAmount CMerkleTx::GetTxOutValue(int i) const {
    if(!Params().IsBTCRWForkHeight(GetTxConfirmHeight())) {
