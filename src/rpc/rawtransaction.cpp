...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
        ...
        for (unsigned int idx = 0; idx < prevTxs.size(); idx++) {
...

            {
                ...
                if (prevOut.exists("amount")) {
                    newcoin.out.SetValue(AmountFromValue(find_value(prevOut, "amount")));
                }
                newcoin.nHeight = 1;
    ...
    if (request.params.size() > 3 && !request.params[3].isNull()) {
        static std::map<std::string, int> mapSigHashValues = {
           ...
           {std::string("ALL|BTCRW_FORK"), int(SIGHASH_ALL|SIGHASH_BTCRW_FORK)},
           {std::string("ALL|BTCRW_FORK|ANYONECANPAY"), int(SIGHASH_ALL|SIGHASH_BTCRW_FORK|SIGHASH_ANYONECANPAY)},
           {std::string("NONE"), int(SIGHASH_NONE)},
           {std::string("NONE|ANYONECANPAY"), int(SIGHASH_NONE|SIGHASH_ANYONECANPAY)},
           {std::string("NONE|BTCRW_FORK"), int(SIGHASH_NONE|SIGHASH_BTCRW_FORK)},
           {std::string("NONE|BTCRW_FORK|ANYONECANPAY"), int(SIGHASH_NONE|SIGHASH_BTCRW_FORK|SIGHASH_ANYONECANPAY)},
           {std::string("SINGLE"), int(SIGHASH_SINGLE)},
           {std::string("SINGLE|ANYONECANPAY"), int(SIGHASH_SINGLE|SIGHASH_ANYONECANPAY)},
           {std::string("SINGLE|BTCRW_FORK"), int(SIGHASH_SINGLE|SIGHASH_BTCRW_FORK)},
           {std::string("SINGLE|BTCRW_FORK|ANYONECANPAY"), int(SIGHASH_SINGLE|SIGHASH_BTCRW_FORK|SIGHASH_ANYONECANPAY)},
        };
